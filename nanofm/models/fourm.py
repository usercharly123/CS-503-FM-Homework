...
class FourM(nn.Module):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.enc_tok_emb = nn.Embedding(self.vocab_size, dim, padding_idx=padding_idx) # TODO: Define the input embedding layer using self.vocab_size
    ...
    def generate_one_modality_roar(
        ...
        dec_input_positions_list = [torch.tensor(positions[i], device=device) for i in range(num_steps)]
        assert len(set(np.concatenate(positions))) == n_tokens_target, "Each position should be present exactly once."
        ...
        for step, k in enumerate(schedule):
            ...
            samples, _ = sample_tokens(predicted_logits, temp, top_k, top_p)    # samples is of shape [k]?
            assert samples.shape == (k,), "The shape of the sampled tokens should be (k,)??"
